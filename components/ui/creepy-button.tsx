...
const SIZE_CLASSES =
    "px-8 py-4 font-display text-sm font-bold uppercase tracking-[0.15em] min-w-[11em]";
...
export const CreepyButton = ({
    ...
    return (
        <button
            className={cn(
                "relative min-w-[9em] rounded-xl cursor-pointer outline-none select-none group tap-highlight-transparent",
                "bg-[#0b0c10] border border-white/15",
                "focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-black focus-visible:ring-white/50",
                className
            )}
            onClick={onClick}
            onMouseMove={(e) => {
        ...
        >
            ...
            <motion.span
                className={cn(
                    "absolute inset-0 block rounded-xl bg-[#16181e] text-white",
            ...
            <span className={cn("block opacity-0", SIZE_CLASSES)}>
                {children}
            </span>
        </button>
    );
